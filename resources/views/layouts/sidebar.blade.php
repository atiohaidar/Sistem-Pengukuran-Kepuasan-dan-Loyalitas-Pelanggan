...
<div x-data="{ sidebarOpen: window.innerWidth >= 768 }" @toggle-sidebar.window="sidebarOpen = !sidebarOpen" class="bg-white shadow-lg transform transition-all duration-300 ease-in-out fixed overflow-hidden"
    ...
    <nav class="mt-8 px-4 mx-2">
        <div class="space-y-2">
            <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="block px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-900 rounded-md text-left">
                {{ __('Dashboard') }}
            </x-nav-link>
            <!-- Manajemen Survei Kepuasan dan Loyalitas Pelanggan with Submenu -->
            <div x-data="{ subOpen: false }">
                <button @click="subOpen = !subOpen" class="flex items-center justify-between w-full px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-900 rounded-md focus:outline-none text-left">
            ...
            <div x-data="{ subOpen: false }">
                ...
                <div x-show="subOpen" x-transition class="ml-4 space-y-1">
                    ...
                    <a href="{{ route('dashboard.customer-evaluation-management.index') }}" class="block px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-md text-left">Kelola Data Evaluasi</a>
                </div>
            </div>
        </div>
    </nav>
    ...
    <div class="absolute bottom-0 left-0 right-0 pb-3 border-t border-gray-200 bg-white">
        <div x-data="{ userMenuOpen: false }" class="px-4 pt-4">
            <button @click="userMenuOpen = !userMenuOpen" class="flex items-center justify-between w-full py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-900 rounded-md focus:outline-none text-left">
            ...
            <div x-show="userMenuOpen" x-transition class="mt-2 space-y-1">
                <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-md text-left">
                    {{ __('Edit Profile') }}
                </a>
