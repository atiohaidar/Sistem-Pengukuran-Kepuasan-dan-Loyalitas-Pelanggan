<x-guest-layout title="Profil Responden - Survei Kepuasan {{ ucfirst($type ?? 'Pelatihan') }}">
    <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-lg rounded-lg overflow-hidden">
                ...
                <form method="POST"
                    action="{{ isset($campaign) ? route('public-survey.submit', ['slug' => $campaign->slug, 'step' => 'profile']) : route('survey.store', ['type' => $type ?? 'pelatihan', 'step' => 'profile']) }}"
                    class="p-6">
                    @csrf
                    @if(isset($campaign))
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select id="pekerjaan" name="pekerjaan"
                                ...
                                <option value="Karyawan swasta" {{ old('pekerjaan', $survey->getProfileData('pekerjaan')) == 'Karyawan swasta' ? 'selected' : '' }}>
                                    Karyawan swasta</option>
                                <option value="Wiraswasta" {{ old('pekerjaan', $survey->getProfileData('pekerjaan')) == 'Wiraswasta' ? 'selected' : '' }}>Wiraswasta
                                </option>
                                <option value="PNS" {{ old('pekerjaan', $survey->getProfileData('pekerjaan')) == 'PNS' ? 'selected' : '' }}>PNS</option>
                                <option value="Pelajar/Mahasiswa" {{ old('pekerjaan', $survey->getProfileData('pekerjaan')) == 'Pelajar/Mahasiswa' ? 'selected' : '' }}>
                                    Pelajar/Mahasiswa</option>
                                <option value="Lainnya" {{ old('pekerjaan', $survey->getProfileData('pekerjaan')) == 'Lainnya' ? 'selected' : '' }}>Lainnya
                                </option>
                            </select>
                            @error('pekerjaan')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select id="domisili" name="domisili"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('domisili') border-red-500 @enderror"
                                required>
                                <option value="">Pilih Domisili</option>
                                <option value="Aceh" {{ old('domisili', $survey->getProfileData('domisili')) == 'Aceh' ? 'selected' : '' }}>Aceh</option>
                                <option value="Sumatera Utara" {{ old('domisili', $survey->getProfileData('domisili')) == 'Sumatera Utara' ? 'selected' : '' }}>Sumatera
                                    Utara</option>
                                <option value="Sumatera Barat" {{ old('domisili', $survey->getProfileData('domisili')) == 'Sumatera Barat' ? 'selected' : '' }}>Sumatera
                                    Barat</option>
                                ...
                                <option value="Sumatera Selatan" {{ old('domisili', $survey->getProfileData('domisili')) == 'Sumatera Selatan' ? 'selected' : '' }}>
                                    Sumatera Selatan</option>
                                <option value="Bengkulu" {{ old('domisili', $survey->getProfileData('domisili')) == 'Bengkulu' ? 'selected' : '' }}>Bengkulu
                                </option>
                                <option value="Lampung" {{ old('domisili', $survey->getProfileData('domisili')) == 'Lampung' ? 'selected' : '' }}>Lampung
                                </option>
                                <option value="DKI Jakarta" {{ old('domisili', $survey->getProfileData('domisili')) == 'DKI Jakarta' ? 'selected' : '' }}>DKI Jakarta
                                </option>
                                <option value="Jawa Barat" {{ old('domisili', $survey->getProfileData('domisili')) == 'Jawa Barat' ? 'selected' : '' }}>Jawa Barat
                                </option>
                                <option value="Jawa Tengah" {{ old('domisili', $survey->getProfileData('domisili')) == 'Jawa Tengah' ? 'selected' : '' }}>Jawa Tengah
                                </option>
                                <option value="DI Yogyakarta" {{ old('domisili', $survey->getProfileData('domisili')) == 'DI Yogyakarta' ? 'selected' : '' }}>DI
                                    Yogyakarta</option>
                                <option value="Jawa Timur" {{ old('domisili', $survey->getProfileData('domisili')) == 'Jawa Timur' ? 'selected' : '' }}>Jawa Timur
                                </option>
                                ...
                                <option value="Nusa Tenggara Barat" {{ old('domisili', $survey->getProfileData('domisili')) == 'Nusa Tenggara Barat' ? 'selected' : '' }}>
                                    Nusa Tenggara Barat</option>
                                <option value="Nusa Tenggara Timur" {{ old('domisili', $survey->getProfileData('domisili')) == 'Nusa Tenggara Timur' ? 'selected' : '' }}>
                                    Nusa Tenggara Timur</option>
                                <option value="Kalimantan Barat" {{ old('domisili', $survey->getProfileData('domisili')) == 'Kalimantan Barat' ? 'selected' : '' }}>
                                    Kalimantan Barat</option>
                                <option value="Kalimantan Tengah" {{ old('domisili', $survey->getProfileData('domisili')) == 'Kalimantan Tengah' ? 'selected' : '' }}>
                                    Kalimantan Tengah</option>
                                <option value="Kalimantan Selatan" {{ old('domisili', $survey->getProfileData('domisili')) == 'Kalimantan Selatan' ? 'selected' : '' }}>
                                    Kalimantan Selatan</option>
                                <option value="Kalimantan Timur" {{ old('domisili', $survey->getProfileData('domisili')) == 'Kalimantan Timur' ? 'selected' : '' }}>
                                    Kalimantan Timur</option>
                                <option value="Kalimantan Utara" {{ old('domisili', $survey->getProfileData('domisili')) == 'Kalimantan Utara' ? 'selected' : '' }}>
                                    Kalimantan Utara</option>
                                <option value="Sulawesi Utara" {{ old('domisili', $survey->getProfileData('domisili')) == 'Sulawesi Utara' ? 'selected' : '' }}>Sulawesi
                                    Utara</option>
                                <option value="Sulawesi Tengah" {{ old('domisili', $survey->getProfileData('domisili')) == 'Sulawesi Tengah' ? 'selected' : '' }}>
                                    Sulawesi Tengah</option>
                                <option value="Sulawesi Selatan" {{ old('domisili', $survey->getProfileData('domisili')) == 'Sulawesi Selatan' ? 'selected' : '' }}>
                                    Sulawesi Selatan</option>
                                <option value="Sulawesi Tenggara" {{ old('domisili', $survey->getProfileData('domisili')) == 'Sulawesi Tenggara' ? 'selected' : '' }}>
                                    Sulawesi Tenggara</option>
                                <option value="Gorontalo" {{ old('domisili', $survey->getProfileData('domisili')) == 'Gorontalo' ? 'selected' : '' }}>Gorontalo
                                </option>
                                <option value="Sulawesi Barat" {{ old('domisili', $survey->getProfileData('domisili')) == 'Sulawesi Barat' ? 'selected' : '' }}>Sulawesi
                                    Barat</option>
                                <option value="Maluku" {{ old('domisili', $survey->getProfileData('domisili')) == 'Maluku' ? 'selected' : '' }}>Maluku</option>
                                <option value="Maluku Utara" {{ old('domisili', $survey->getProfileData('domisili')) == 'Maluku Utara' ? 'selected' : '' }}>Maluku
                                    Utara</option>
                                <option value="Papua Barat" {{ old('domisili', $survey->getProfileData('domisili')) == 'Papua Barat' ? 'selected' : '' }}>Papua Barat
                                </option>
