<x-guest-layout title="Kritik dan Saran - Survei Kepuasan Produk">
	...
	<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		<div class="bg-white shadow-lg rounded-lg overflow-hidden">
			<div class="bg-gradient-to-r from-red-600 to-pink-600 px-6 py-4">
				...
				<p class="text-red-100 mt-1">Dengan anda mengisi kritik dan saran ini, diharapkan kualitas produk ini akan semakin baik dan sesuai dengan kebutuhan Anda.</p>
			</div>

			<form method="POST" action="{{ route('survey.produk.store', ['step' => 'feedback']) }}" class="p-6">
				@csrf
				...
				<div class="space-y-8">
					...
					<!-- Question {{ $counter }}: Tema/Judul -->
					<div class="bg-gray-50 p-6 rounded-lg">
						<h3 class="text-lg font-semibold text-gray-800 mb-4">{{ $counter }}. {{ $question }}</h3>
						<textarea name="tema_judul" rows="4"
								  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 resize-vertical"
								  placeholder="Contoh: Fitur baru, Perbaikan kualitas, dll.">{{ $feedbackData['tema_judul'] ?? old('tema_judul') }}</textarea>
						<p class="text-sm text-gray-500 mt-2">Maksimal 500 karakter</p>
					</div>
					@elseif($key === 'bentuk_pelatihan')
					<!-- This section repurposed for product delivery preferences -->
					<div class="bg-gray-50 p-6 rounded-lg">
						<h3 class="text-lg font-semibold text-gray-800 mb-4">{{ $counter }}. {{ $question }}</h3>
						...
						<div class="grid md:grid-cols-2 gap-4">
							<label class="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
								...
								<div>
									...
									<p class="text-sm text-gray-600">Produk tersedia secara daring</p>
								</div>
							</label>
							...
							<label class="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
								...
								<div>
									...
									<p class="text-sm text-gray-600">Produk tersedia dalam bentuk fisik</p>
								</div>
							</label>
							...
							<label class="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
								...
								<div>
									...
									<p class="text-sm text-gray-600">Peluncuran/preview produk secara live</p>
								</div>
							</label>
							...
							<label class="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
								...
								<div>
									...
									<p class="text-sm text-gray-600">Materi produk dalam bentuk pembelajaran mandiri</p>
								</div>
							</label>
						</div>
					</div>
