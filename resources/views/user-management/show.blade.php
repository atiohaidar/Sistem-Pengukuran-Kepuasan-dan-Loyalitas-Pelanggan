    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
                ...
                <!-- Edit Mode -->
                <form id="user-edit" method="POST" action="{{ route('user-management.update', $user->id) }}"
                    ...
                    <div>
                        ...
                        <select name="status"
                            ...
                            <option value="approved" {{ $user->status === 'approved' ? 'selected' : '' }}>Disetujui
                            </option>
                    ...
                    <div class="md:col-span-2 flex justify-end space-x-3">
                        <button type="button" onclick="toggleEditUser()"
                            class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">Batal</button>
                        <button type="submit"
                            class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">Simpan</button>
                    </div>
                </form>
            </div>
            ...
            @if($user->umkm)
                ...
                <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
                    ...
                    <!-- Edit Mode -->
                    <form id="umkm-edit" method="POST" action="{{ route('user-management.update-umkm', $user->umkm->id) }}"
                        class="hidden grid grid-cols-1 md:grid-cols-2 gap-6">
                        @csrf
                        @method('PUT')
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Nama Usaha</label>
                            <input type="text" name="nama_usaha" value="{{ $user->umkm->nama_usaha }}"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Kategori Usaha</label>
                            <input type="text" name="kategori_usaha" value="{{ $user->umkm->kategori_usaha }}"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700">Deskripsi</label>
                            <textarea name="deskripsi" rows="3"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ $user->umkm->deskripsi }}</textarea>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700">Alamat</label>
                            <textarea name="alamat" rows="3"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ $user->umkm->alamat }}</textarea>
                        </div>
                        <div class="md:col-span-2 flex justify-end space-x-3">
                            <button type="button" onclick="toggleEditUmkm()"
                                class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">Batal</button>
                            <button type="submit"
                                class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">Simpan</button>
                        </div>
