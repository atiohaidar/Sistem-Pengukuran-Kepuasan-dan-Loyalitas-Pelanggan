<x-guest-layout title="Penilaian Persepsi - Survei Kepuasan {{ ucfirst($type ?? 'Pelatihan') }}">
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-lg rounded-lg overflow-hidden">
                <div class="bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-4">
                    ...
                    <p class="text-purple-100 mt-1">Berikan penilaian sesuai dengan pengalaman Anda terhadap kinerja
                        kualitas {{ $type === 'produk' ? 'produk/layanan' : 'layanan' }} yang diberikan</p>
                </div>

                <form method="POST"
                    action="{{ isset($campaign) ? route('public-survey.submit', ['slug' => $campaign->slug, 'step' => 'persepsi']) : route('survey.store', ['type' => $type ?? 'pelatihan', 'step' => 'persepsi']) }}"
                    class="p-6">
                    @csrf
                    @if(isset($campaign))
                        <input type="hidden" name="step" value="persepsi">
                    @endif
