            ...
            <div class="mb-6">
                @if(!empty($campaign))
                    <div class="flex items-start justify-between">
                        <div>
                            <a href="{{ route('grafik.dashboard-campaign', $campaign->id) }}"
                                class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-indigo-500 to-indigo-600 border border-indigo-500 rounded-lg shadow-sm text-sm font-medium text-white hover:from-indigo-600 hover:to-indigo-700 hover:border-indigo-600 transition-all duration-200">
                            ...
                            <div class="mt-3">
                                ...
                                <p class="text-sm text-gray-600">{{ $campaign->responses_count }} responden •
                        ...
                        <div class="text-right">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $campaign->getTypeBadgeColor() }}-100 text-{{ $campaign->getTypeBadgeColor() }}-800">
                ...
                @else
                    <a href="{{ route($type === 'produk' ? 'dashboard.produk' : 'dashboard.pelatihan') }}"
                        class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-indigo-500 to-indigo-600 border border-indigo-500 rounded-lg shadow-sm text-sm font-medium text-white hover:from-indigo-600 hover:to-indigo-700 hover:border-indigo-600 transition-all duration-200 transform hover:scale-105">
                        <i class="fas fa-arrow-left mr-2 text-lg"></i>
                    ...
                    @php
                        ...
                        function renderILPTable($ilpPercentage, $ilpInterpretation)
                        {
                            ...
                            foreach ($scales as $scale) {
                                $isCurrent = false;
                                if (
                                    ($ilpPercentage >= 81 && $scale['range'] === '81-100') ||
