...
<div class="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white shadow rounded-lg p-6">
            ...
            <p class="text-gray-600 mb-6">Silahkan melakukan pembobotan kelipatan 5 untuk item yang ingin diprioritaskan. Total bobot dari item yang diisi harus sama dengan 100.</p>

            <!-- Total Display -->
            <div class="bg-gray-100 p-4 rounded-lg mb-6 sticky top-4">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-medium text-gray-900">Total Bobot</h3>
                    <span id="total-display" class="text-2xl font-bold {{ collect($data['priority'] ?? [])->sum() == 100 ? 'text-green-600' : 'text-red-600' }}">
                        {{ collect($data['priority'] ?? [])->sum() }}/100
                    </span>
                </div>
            </div>

            <form id="priority-form" action="{{ route('customer-management-evaluation.store-priority') }}" method="POST">
                @csrf
