...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const gapContainer = document.getElementById('gap-chart-container');
    if (gapContainer) {
        gapObserver.observe(gapContainer);
    }

    // Observe std dev chart
    const stdDevObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                renderChartAsync(createStdDevChart, stdDevData, 'stddev-chart-container');
                stdDevObserver.unobserve(entry.target);
            }
        });
    }, observerOptions);

    const stdDevContainer = document.getElementById('stddev-chart-container');
    if (stdDevContainer) {
        stdDevObserver.observe(stdDevContainer);
    }
